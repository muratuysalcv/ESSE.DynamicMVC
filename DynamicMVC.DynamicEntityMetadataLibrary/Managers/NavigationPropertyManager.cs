...
namespace DynamicMVC.DynamicEntityMetadataLibrary.Managers {
    public class NavigationPropertyManager : INavigationPropertyManager {
        ...
        public DynamicForiegnKeyPropertyMetadata GetDynamicForiegnKeyPropertyMetadata(DynamicEntityMetadata dynamicEntityMetadata, DynamicComplexPropertyMetadata dynamicComplexPropertyMetadata)
        {
            ...
            var first = dynamicForiegnKeyPropertyMetadatas.FirstOrDefault();
            if (first != null)
            {
                if (first.DynamicEntityMetadata.ReflectedClass.Name == "fi_invoice")
                {
                    var propertyName = first.PropertyName();
                    var dynProperyName = dynamicComplexPropertyMetadata.PropertyName();
        ...
        public List<string> GetForiegnKeyNameByCollectionProperty(DynamicEntityMetadata dynamicEntityMetadata, string typeName, DynamicCollectionEntityPropertyMetadata dynamicCollectionEntityPropertyMetadata)
        {
            ...
            else
            {
                ...
                if (dynamicEntityMetadata.DynamicPropertyMetadatas.All(x => !fkName.Contains(x.PropertyName())))
                {
        ...
        {
            var potentialForeignKeyNamesList = reflectedClass.ReflectedProperties.Where(x => x.IsComplex).Select(x => _namingConventionManager.GetForiegnKeyByComplexProperty(reflectedClass.Name, x.Name)).ToList();
            var potentialForeignKeyNames = new List<string>();
            foreach (var item in potentialForeignKeyNamesList)
            {
                potentialForeignKeyNames.AddRange(item);
            }
