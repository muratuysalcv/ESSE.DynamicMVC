...
namespace DynamicMVC.Core.DynamicMVC.Managers {
    ...
    public class DynamicMvcManager : IDynamicMvcManager {
        ...
        public void SetDynamicRoutes(RouteCollection routeCollection) {
            ...
            var firstRoute = routeCollection[0];
            newRouteCollection.Add(firstRoute);

            // TODO : .NET CONVERSION REQUIRED
            ...
            //        defaults: new { controller = dynamicEntityMetadata.TypeName(), action = "Index", typeName = dynamicEntityMetadata.TypeName() }
            //        );
            //    } else {
            ...
            //        name: "Dynamic" + dynamicEntityMetadata.TypeName(),
            //        url: dynamicEntityMetadata.TypeName() + "/{action}/{id}",
            //        defaults: new { controller = "Dynamic", action = "Index", id = UrlParameter, typeName = dynamicEntityMetadata.TypeName() }
            //        );
            ...
            for (int i = 0; i < routeCollection.Count; i++) {
                if (routeCollection[i] != firstRoute) {
                    newRouteCollection.Add(routeCollection[i]);
                }
            }

            routeCollection = new RouteCollection();

            for (int i = 0; i < newRouteCollection.Count; i++) {
                var newRoute = newRouteCollection[i];
